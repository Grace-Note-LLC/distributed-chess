...
protected:
    void SetUp() override {
        ...
        boardRook = new Board();
        boardRook->fillEmpty();
        moveGenRook = new MoveGenerator();

        boardBishop = new Board();
        boardBishop->fillEmpty();
        moveGenBishop = new MoveGenerator();
    }
    ...
    std::vector<Move> bothMoveGenSetPieceRF(int rank, int file) {
        auto moves = this->moveGenRook->generatePieceMoves(boardRook, Board::WHITE_ROOKS);
        auto moves2 = this->moveGenBishop->generatePieceMoves(boardBishop, Board::WHITE_BISHOPS);
        moves.insert(moves.end(), moves2.begin(), moves2.end());
        return moves;
    }

    std::vector<Move> bothBlackMoveGenSetPieceRF(int rank, int file) {
        auto moves = this->moveGenRook->generatePieceMoves(boardRook, Board::BLACK_ROOKS);
        auto moves2 = this->moveGenBishop->generatePieceMoves(boardBishop, Board::BLACK_BISHOPS);
        moves.insert(moves.end(), moves2.begin(), moves2.end());
        return moves;
    }
...
TEST_F(QueenTest, QueenMoveGeneration_All) {
    for (int rank = 0; rank < 8; rank++) {
        for (int file = 0; file < 8; file++) {
            ...
            std::vector<Move> moves = moveGen->generatePieceMoves(board, Board::WHITE_QUEEN);
